...
public class CustomListAdapter extends BaseAdapter {
    ...
    public CustomListAdapter(Activity activity, List<Capteur> capteurItems) {
        ...
        updateURL = "http://"+Constante.IP_SERVEUR+"/hpm/update_sensor.php";
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        //Log.v("CustomListAdapter", "GET VIEW");
        ...
        onoff.setOnClickListener(
                new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        ...
                        Log.v(Constante.TAG, "Capteur modifié "+ m.getIdc());
                        params.add(new BasicNameValuePair("id_capteur", m.getIdc()));
                        if (isChecked) {
                            Log.v(Constante.TAG, "Nouvel état -> 1");
                            //m.setEtat(true);
                            params.add(new BasicNameValuePair("etat_demande", "1"));
                        } else {
                            Log.v(Constante.TAG, "Nouvel état -> 0");
                            //m.setEtat(false);
